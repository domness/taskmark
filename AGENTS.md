...
- This is Taskmark (formerly Local Todo), an Apple-native, macOS-first, local-first task manager and CLI built with Swift 6. The `localtodo` CLI and existing internal target names are retained. Vault schema 2 uses `.config/` for all metadata and shared preferences; the earlier development layout has no migration, as explicitly requested by the user.
