...
Project and area context-menu deletion first removes matching metadata references, then deletes the collection file. Project removal clears `project` on all assigned tasks, including done/canceled tasks, and in saved filters. Area removal clears `area` on tasks, projects, and saved filters. Delete Tag removes the exact, case-sensitive tag from tasks, projects, areas, and saved-filter criteria. Tasks and other collections are retained; status, completion timestamps, notes, unrelated metadata, and unknown keys survive. Edited entities receive a new `updated_at`. Removing a saved-filter criterion broadens that filter; its other criteria and notes remain intact.

Organization cleanup is a deliberately resumable sequence, **not** an atomic multi-file transaction: validate the complete plan first, apply coordinated per-file atomic edits with expected-field checks, and delete the collection last with its original revision and a fresh reference scan. Malformed/unreadable entity or filter files block planning; a conflict or I/O failure stops execution and reports partial progress. Completed changes remain on disk, the collection is retained if its deletion has not succeeded, and retry replans the remaining work. External readers may observe intermediate states, and new external assignments may block the final deletion. No rollback overwrites external work. A process crash leaves the completed per-file changes; reopening and repeating removal resumes cleanup, while session-local Undo is lost.

One app Undo action reverses the completed steps, restoring the exact collection bytes before restoring references. Edited entities restore only the affected organization field while preserving unrelated current fields and body bytes; overlapping field changes cause a conflict. Saved-filter history checks the expected definitions and preserves unknown metadata. Undo/Redo also stops and reports partial progress on failure. Restoration refuses occupied collection paths. The low-level `delete` API retains reference-protected deletion; the app uses the shared Markdown cleanup APIs explicitly.
