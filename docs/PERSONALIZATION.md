...
Drag a project onto another project to insert it before the target; areas work independently. Right-click for **Move Up**, **Move Down**, or **Restore Default Order** (also accessible through native contextual-menu keyboard navigation).
...
Turn off **View Options → Appearance → Use Vault Stylesheet** to use built-in appearance for this open vault session. Opening a vault enables its stylesheet again.
...
`px` values map to native logical points. There is no browser CSS engine: arbitrary selectors, layout rules, imports, URLs, variables, named colors and media queries are unsupported. Titles and surfaces retain native colors; focus rings and textual priority cues remain. Check custom colors against your preferred appearance and contrast settings.
...
- `make format && make check`: passed. Formatting and strict lint were clean; 118 Swift package tests and 74 macOS app tests passed; the generated LocalTodoApp Debug scheme built successfully with `CODE_SIGNING_ALLOWED=NO`.
