...
Right-click a collection for **Delete Project** or **Delete Area**, or a tag for **Delete Tag**. Pending edits save first. Removal clears matching assignments from all tasks, including completed/canceled tasks, and saved filters; area removal also clears assignments on projects, and tag removal includes projects and areas. Tasks and their completion history remain intact. Saved filters keep their other criteria. Deleting the open collection/tag returns navigation to Inbox. One session-local Undo restores the removed collection and assignments; Redo removes them again. If a file conflict or write failure interrupts cleanup, the app reports partial progress, retains completed changes for Undo, and lets you retry the remaining cleanup. See the [file contract](FILE_FORMAT.md#task-copies-and-reversible-deletion).
...
Turn off **View Options → Appearance → Use Vault Stylesheet** or **Settings → Theme → Apply vault stylesheet** to use the selected built-in theme. This preference is stored with each vault, alongside System/Light/Dark appearance and the Taskmark, Slate, Forest, Sand, Catppuccin and Dracula palette choice. See [SETTINGS.md](SETTINGS.md) and the full [theme guide](THEMES.md).
