...
- Custom task ordering uses native List moves within the current group. Own-process, vault-session-bound transferable task drags assign sidebar projects/areas or append a tag. Automatic sorts use the row; Custom mode uses a separate trailing handle so native row reordering retains its hit region. Assignments use revision-checked, field-specific transitions and register Undo only after successful persistence.
