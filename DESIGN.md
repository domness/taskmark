...
- **Workspace**: Two-column sidebar and opaque task canvas by default, with a trailing inspector that opens when a task row is clicked and can also be toggled from the window toolbar. The reading canvas is capped at 760 points. Inbox appears above Today; Tags and Priorities start collapsed. Search, capture and display controls belong to the native window toolbar. Search reveals its field on the Search route. Project notes appear below the project heading (three-line reading summary; Edit Project opens the complete source).
...
- **Organization**: The composed task inspector places a completion control beside the title, followed by notes and checklist. Small secondary section headings divide Planning (Scheduled, Deadline, Status, Priority) from Organization (Project, Area, Tags, Repeat). Quiet labels align left, flexible property values align right, and native menu pickers are borderless with neutral tint. Organization stays directly visible; tags wrap within the value column with an add popover, and advanced recurrence fields appear only for the selected mode. File details start collapsed and contain the exact path, timestamps, file actions and idle Saved indicator; pending saves, validation problems, conflicts and file recovery remain outside the disclosure. New Today/Upcoming/Area display defaults group by project; rows suppress the grouping's repeated label. Area and tag row metadata defaults off, with existing saved choices preserved and View Options available.
